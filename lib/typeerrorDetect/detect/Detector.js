...
class Detector {
    ...
    dealWithDelete () {
        this.records.getAll()
                    .filter(rcd => rcd.entryType == 'DELETE')
                    .forEach(del => {
    ...
    _detect () {
        ...
        while (this.undefRcd.length > 0) {
            logger.info('length of undef records: %d', this.undefRcd.length);
            //for test
            this.printRecords('undef records', this.undefRcd);
    ...
    findSuspiciousUse (rcd, name) {
        return (rcd.name == name || rcd.prop == name )&& 
    ...
    filter () {
        //policy 1: only 1 local declared
        //for test
        /*logger.warn('before filter');
        this.printReports();*/
        this.reports = this.reports.filter(rpt => rpt.declareTimes < 2);
...
class Report {
    constructor (undef, use, isPropagate) {
        ...
        this.declareTimes = (undef.isDeclaredLocal? 1 : 0 ) + (use.isDeclaredLocal? 1 : 0);
    }
