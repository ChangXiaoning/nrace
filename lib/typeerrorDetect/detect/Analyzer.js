...
class Analyzer {
            ...
            if (loadRcd) {
                switch (loadRcd.entryType) {
                    ...
                    case 'CREATE_OBJ':
                        ...
                        type = 'assign_1_' + record.lineno;
                        break;
                    //case x = y (y is an object)
                    case 'READ':
                        dest = me.generateNodeName(loadRcd);
                        type = 'assign_2_'+ record.lineno;
                        break
            ...
            switch (val) {
                case '0':
                    ...
                    if (loadRcd)
                        ...
                        //case x = 1
                        dest = me.generateValNodeName(record);
                    type = 'assign_3_' + record.lineno;
                    break;
                case '-2':
                    dest = me.generateValNodeName(record);
                    type = 'assign_4_' + record.lineno;
                    break;
                case '-3':
                    dest = me.generateValNodeName(record);
                    type = 'assign_5_' + record.lineno;
                    break;
            }
        }
    ...
    generateNodeName (record, isUseBasename = true) {
        var res = null;
        switch (record.entryType) {
            ...
            case 'DELETE':
                if (isUseBasename)
                    ...
                    res = ['obj_' + record.name, record.event].join('@');
                break;
            default:
