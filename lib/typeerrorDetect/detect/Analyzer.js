...
class Analyzer {
    ...
    intraEventAnalyze () {
        ...
        for (let variable of outline) {
            ...
            let value_node = null;
            switch (value.type) {
                //undefined value belongs to expression
                case 'expression':
                    let identifiers = [];
                    ...
                    dfatool.walk(ast, function (node) {
                        ...
                        switch(node.type) {
                            case "Identifier":
                                ...
                                let name = node.name =='undefined'? '#' + node.name : node.name;
    ...
    computeLeaf () {
        ...
        events.forEach(event => {
            ...
            let leaves = [];
            _nodes.forEach(node => {
                if (!node.startsWith('obj_') && !node.startsWith('val_') && !me.isUndefinedNode(node)) {
