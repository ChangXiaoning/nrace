...
class BenignChecker {
    ...
    findBinary () {
        ...
        for (let binary of binaries) {
            ...
            //match left operand with result writing
            let candidateResultOperations = sameEventRecords.filter(rcd => rcd.name == leftOp.name && rcd.val == binary.result && rcd.lineno > binary.lineno);
            if (leftOp.prop)
                candidateResultOperations = candidateResultOperations.filter(rcd => rcd.prop == leftOp.prop && rcd.entryType == 'PUTFIELD');
            let resultOp = candidateResultOperations.length > 0 ? candidateResultOperations[0] : null;
