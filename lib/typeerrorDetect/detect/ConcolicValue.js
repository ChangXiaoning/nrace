...
class AnnotatedExecution {
    ...
    addSymbolic (concrete, symbolic, isFieldAccess, valIsObject, e, lineno, isDeclaredLocal, contextIid, entryType) {
        //if (isDeclaredLocal) return;
        if (lineno == 9768)
            console.log('bug');
        let me = this;

        //identify primitive values and objects
        let cval = null;
        if (valIsObject == 'true') {
            cval = me.getConcolicValue(concrete, valIsObject);
            if (isFieldAccess) {
                let base = symbolic.base;
                let prop = symbolic.prop;
        
                let c_base = me.getConcolicValue(base, 'true');
                //TODO: ignore the case: return {}; {}.field
                if (c_base == null)
                    return
                ...
                //it is possible that the cval has more than one annotated values
                if (entryType == "READ") {
                    for (let symbol of cval.symbolic) {
                        let local = symbol.local;
                        if (me.isPointerAccessible(local, contextIid, local.name)) {
                            let s = { local: local, access_path: symbol.access_path,
                                event: e, lineno: lineno, }
                            if (!me.isExistSymbol(cval, s))
                                cval.symbolic.push(s);
                        }
                    }
                }
                var scope = null;
                ...
                if (scope == null) {
                    //use <objId, name(this)> to identify 'this'
                    if (symbolic == THIS)
                        scope = concrete;
                    else
                        scope = BUILDIN_SCOPE;
                }
                var ts = { local: {scope: scope, name: symbolic},
                    access_path: [], event: e, lineno: lineno }
                if (!me.isExistSymbol(cval, ts))
                    cval.symbolic.push(ts);
            }
        } else {
            cval = new ConcolicValue(concrete, symbolic, isFieldAccess, valIsObject, e, me, lineno, isDeclaredLocal, contextIid);
        }
    ...
    /**
     * 
     * @return {Map}: cval -> overwritePointers <Array>
     * cval is not pointed by pointers in overwritePointers
     */
    ...
    /**
     * check whether the given pointer is accessible in current context (currentCtxIid)
     * @param {Object} pointer: <scope, name>
     * @param {*} currentCtxIid
     * @param {*} targetName 
     */
    isPointerAccessible (pointer, currentCtxIid, targetName) {
        //deal with 'this' dynamic binding
        if (targetName == THIS)
            return true;
        let me = this;
        let ctx = currentCtxIid;
        let c = ctx.split('-')[0];
        let givenScope = pointer.scope.split('-')[0];

        while (ctx) {
            if (me.isGivenCtxHasDeclaredVar(ctx, targetName)) {
                if (ctx.split('-')[0] == givenScope)
                    return true;
                else
                    return false;
            }
