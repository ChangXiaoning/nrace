...
class AnnotatedExecution {
    ...
    addSymbolic (concrete, symbolic, isFieldAccess, valIsObject, e, lineno, isDeclaredLocal) {
        ...
        if (isFieldAccess) {
            ...
            cval.symbolic.push({ local: symbolic, access_path: [], event: e, lineno: lineno });
        }
        return cval;
    };
    ...
    toString (symbolic, includeEv = true) {
        ...
        if (Array.isArray(symbolic)) {
            ...
            if (includeEv)
                ...
                res.push([symbolic.local, ...symbolic.access_path].join(Connector));
        }
        return res;
    }
}

function ConcolicValue (concrete, symbolic, isFieldAccess, valIsObject, e, execution, lineno, isDeclaredLocal) {
    //if (concrete == '115')
        //console.log('bug');
    ...
    if (isFieldAccess) {  
        ...
        this.symbolic.push({ local: symbolic, access_path: [], event: e, lineno: lineno });
    }
