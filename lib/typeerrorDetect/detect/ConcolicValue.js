...
function ConcolicValue (concrete, symbolic, isFieldAccess, valIsObject) {
...
ConcolicValue.addSymbolic = function (concrete, symbolic, isFieldAccess, valIsObject) {
...
ConcolicValue.backwardUpdateSymbolic = function (ctarget) {
    ...
    let index_s_ym = [symbolic.local, ...symbolic.access_path].join('*');
    for (let cval of ConcolicValue.collection) {
        if (cval == ctarget)
            continue;
...
ConcolicValue.isOverwritten = function (s_symbol, cval) {
    let isOverwritten = false;
    for (let ap of cval.symbolic) {
        let s_ap = [ap.local, ...ap.access_path].join('*');
        if (s_ap == s_symbol) {
            isOverwritten = true;
            break;
        }
...
ConcolicValue.toString = function (symbolic) {
    let res = [];
    if (Array.isArray(symbolic)) {
        for (let symbol of symbolic) {
            res.push([symbol.local, ...symbol.access_path].join('*'));
        }
    } else {
        res.push([symbolic.local, ...symbolic.access_path].join('*'));
    }
    return res;
}
