...
ConcolicValue.backwardUpdateSymbolic = function (ctarget) {
    ...
    for (let cval of ConcolicValue.collection) {
        if (cval == ctarget)
            continue;
        ConcolicValue.overwrite(ctarget, cval);
    }
}
 ...
 * @param {*} cval_b 
 */
ConcolicValue.overwrite = function (cval_a, cval_b) {
    let s_indexSym = ConcolicValue.toString(cval_a.symbolic, false);
    for (let sym of cval_b.symbolic) {
        let s_sym = ConcolicValue.toString(sym, false)[0];
        ...
        if ( idx != -1 ) {
            ...
            if (cval_a.symbolic[idx].event == sym.event)
