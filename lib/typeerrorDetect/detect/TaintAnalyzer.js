...
class Analyzer {
    ...
    init () {
        ...
        this.benignChecker = new BenignChecker(this.asyncObjects, this.records, this.appPath, CONDITIONAL_BACKTRACK_NUM);
    ...
    functionEnter (record) {
        let iid = record.iid;
        this.func_execution_count[iid] = (this.func_execution_count[iid] || 0) + 1;
        let func_id = [iid, this.func_execution_count[iid]].join('-');
        this.current_func_iid.push(func_id);
    }
    ...
    detectGeneralRace (idxRcd, cval) {
        ...
        if ((idxRcd.isDeclaredLocal))
            return;
        //if (idxRcd.lineno == 912)
            //console.log('detect bug');
        let lineno = idxRcd.lineno;
        let writeEntryType = ['WRITE', 'PUTFIELD', 'DELETE'];
        let readEntryType = ['READ', 'GETFIELD'];
        let targetEntryType = readEntryType.indexOf(idxRcd.entryType) > -1 ? writeEntryType : [...writeEntryType, ...readEntryType];
        let me = this;
        ...
        if (objs.length >= 2) {
            ...
            if (othervalues.length > 0) {
                ...
                for (let value of othervalues) {
                    ...
                    if (targetEntryType.indexOf(otherRcd.entryType) > -1) {
    ...
    detectActionRace () {
        let me = this;
        let resource_records_map = this.classifyActionsByResource();
        for (let resource in resource_records_map) {
            let records = resource_records_map[resource];
            for (let i = 0; i < records.length - 1; i++) {
                let fsRcd_i = records[i];
                for (let j = i + 1; j < records.length; j++) {
                    let fsRcd_j = records[j];
                    if (this.matchPattern(fsRcd_i, fsRcd_j, FS_PATTERN)) {
                        let isRace = false;
                        //if (fsRcd_i.lineno == 809 && fsRcd_j.lineno == 782)
                            //console.log('bug');
                        if (fsRcd_i.isAsync === '1' && fsRcd_j.isAsync === '1') {
                            if (!me.rg.happensBeforeWithGraphLib(fsRcd_i.id, fsRcd_j.id) && !me.rg.happensBeforeWithGraphLib(fsRcd_j.id, fsRcd_i.id))
                                isRace = true;
                        } else if (fsRcd_i.isAsync === '0' && fsRcd_j.isAsync === '0') {
                            if (fsRcd_i.event == fsRcd_j.event) isRace = false;
                            else if (!me.rg.happensBeforeWithGraphLib(fsRcd_i.event, fsRcd_j.event) && !me.rg.happensBeforeWithGraphLib(fsRcd_j.event, fsRcd_i.event))
                                isRace = true;
                        } else {
                            //suppose fsRcd_i is action
                            let tmp = null;
                            if (fsRcd_i.isAsync === '0') {
                                tmp = fsRcd_i;
                                fsRcd_i = fsRcd_j;
                                fsRcd_j = tmp;
                            }
                            if (!me.rg.happensBeforeWithGraphLib(fsRcd_i.id, fsRcd_j.event) && !me.rg.happensBeforeWithGraphLib(fsRcd_j.event, fsRcd_i.id))
                                isRace = true;
                        }
                        if (isRace) {
                            let report = new Report(fsRcd_i, fsRcd_j, resource, fsRcd_i.entryType + '-' + fsRcd_j.entryType);
                            this.fs_reports.push(report);
                        }
                    }
                }
            }
    ...
    classifyActionsByResource () {
        ...
        let records = this.records.getAll();
        let file_entryType = Object.keys(FS_PATTERN);
        for (let rcd of records) {
            if (file_entryType.indexOf(rcd.entryType) > -1) {
                let resource = rcd.resource;
                if (!res[resource]) res[resource] = [];
                res[resource].push(rcd);
            }
        }
        return res;
    }

    matchPattern (rcdi, rcdj, pattern) {
        let res = false;
        let candidateType = pattern[rcdi.entryType];
        if (candidateType) {
            if (candidateType.indexOf(rcdj.entryType) > -1)
                res = true;
        }
        return res;
    }
    ...
    printReports () {
        ...
        info += '\n*** FS BUG REPORTS ***\n';
        count = 0;
        for (let rpt of this.fs_reports) {
            count++;
            info += '[' + count + ']' + rpt.toString() + '\n';
        }
...
class Report {
    ...
    toString () {
        ...
        if (this.benignInfo)
            res += "Benign info: " + printObj(this.benignInfo, ["benignPattern", "type"]) + "\n";
        return res;
    }
