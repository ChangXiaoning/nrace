            ...
            switch (entryType) {
                case 'WRITE':
                    if (!(ConcolicValue.searchConcolicValue(val, record.valIsObject)))
                        cval = new ConcolicValue(val, record.name, false, record.valIsObject, e);
                    else {
                        cval = ConcolicValue.addSymbolic(val, record.name, false, record.valIsObject, e);
                    }
                    break;
                case 'DELETE':
                case 'PUTFIELD':
                    ...
                    if (!(ConcolicValue.searchConcolicValue(val, record.valIsObject)))
                        cval = new ConcolicValue(val, {base: baseObjId, prop: record.prop}, true, record.valIsObject, e);
                    else
                        cval = ConcolicValue.addSymbolic(val, {base: baseObjId, prop: record.prop}, true, record.valIsObject, e);
                    break;
                case 'GETFIELD':
                    baseObjId = record.name;
                    if (!(ConcolicValue.searchConcolicValue(val, record.valIsObject)))
