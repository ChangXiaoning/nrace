...
class Analyzer {
        ...
        for (let record of records) {
            ...
            if (cval) {
                ...
                for (let symbol of cval.symbolic) {
                    ...
                    let obj = {concrete: cval.concrete, event: symbol.event, valIsObject: cval.valIsObject};
    ...
    detect (s_symbolic_list) {
        ...
        objs = unique(objs);
        if (objs.length >= 2 && objs.indexOf('*U*') != -1) 
            this.reports.push()
            //TODO: report event of alias pointer, so need to add
            //event information in Point2
    }
