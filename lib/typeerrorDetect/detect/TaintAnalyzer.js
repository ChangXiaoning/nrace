            ...
            switch (entryType) {
                ...
                case 'READ':
                    if (!(me.annotatedExecution.searchConcolicValue(val, record.valIsObject)))
                        ...
                        cval = me.annotatedExecution.addSymbolic(val, record.name, false, record.valIsObject, e, record.lineno, isDeclaredLocal, me.getCurrentCtxIid());
                    }
                    break;
                case 'DELETE':
                case 'PUTFIELD':
                case 'GETFIELD':
                    ...
                    if (baseObjId != val) {
                        if (!(me.annotatedExecution.searchConcolicValue(val, record.valIsObject)))
                            ...
                            cval = me.annotatedExecution.addSymbolic(val, {base: baseObjId, prop: record.prop}, true, record.valIsObject, e, record.lineno, isDeclaredLocal, me.getCurrentCtxIid());
                        }
                    } else {
...
class Report {
    ...
    toString () {
        ...
        this.tuple.forEach(rcd => {
            res += printObj(rcd, ['name', 'entryType', 'event', 'iid', 'location', 'lineno', 'val', 's_symbolic', 'valIsObject']);
            res += '\n';
        });
        res += 'pointers: ';
