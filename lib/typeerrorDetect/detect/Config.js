...
VAR_RACE_PATTERNS.VAR_$W = function (first, second){
    if (first.entryType == 'WRITE' && second.entryType == 'WRITE' || first.entryType == 'PUTFIELD' && second.entryType == 'PUTFIELD' || first.entryType == 'DELETE' && second.entryType == 'PUTFIELD' || first.entryType == 'PUTFIELD' && second.entryType == 'DELETE'){
        return true;
    }
    return false;
}
...
VAR_RACE_PATTERNS.VAR_WR = function (first, second){
    ...
    || first.entryType == 'DELETE' && second.entryType == 'GETFIELD' || first.entryType == 'GETFIELD' && second.entryType == 'DELETE'){
        return true;
    }
    return false;
}
...
FP_FILTERS.GLOBAL = function (report) {
    let rcd1 = report.tuple[0];
    let rcd2 = report.tuple[1];
    return rcd1.event != '1' && rcd2.event != '1';
}

FP_FILTERS.FILE = function (report) {
    let rcd1 = report.tuple[0];
    let rcd2 = report.tuple[1];
    if (rcd1.resource)
        return rcd1.resource != '1';
    return true;
}

FP_FILTERS.WR = function (report) {
    let rcd1 = report.tuple[0];
    let rcd2 = report.tuple[1];
    return !(VAR_RACE_PATTERNS.VAR_WR(rcd1, rcd2) && rcd1.val == rcd2.val && rcd1.valIsObject == rcd2.valIsObject);
}
