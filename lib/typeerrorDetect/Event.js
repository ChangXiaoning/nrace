...
EventManager.prototype.enhanceTrace = function () {
    ...
    this.events.forEach(function (id) {
        var event = this.events[id];
        //TODO: FSREQWRAP?
        if (event.resourceType == Event.GLOBAL_RESOURCETYPE) 
            continue;
        if (!this.events[id].hasOwnProperty('prior'))
            continue;
        var targetEvent = id,
            priorEvent = this.events[id].prior,
            virtualEvent = new VirtualEvent(targetEvent, priorEvent);
        this.virtualEvents.set(id, VirtualEvent);
    })
};
