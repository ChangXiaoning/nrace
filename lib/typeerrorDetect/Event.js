 ...
 */
Event.prototype.addEdge = function (edgeType, nextEventId) {
    var debug = false;
    ...
    if (debug) {
        debugHelper('addEdge for #' + this.id + ', next: ' + nextEventId);
    }
...
/**
 * Given the event trace recorded by async_hooks,
 * we collect events without event triggering.
 * Therefore, we model event triggering,
 * except for the global event and asynchronous file IO,
 * Save all these virtual events into this.virtualEvents,
 * indexed by the id of its follower (target) callback
 * TODO: whether it is a problem if the file IO has no triggering event
 * pay attention.
 * Limitation: we assume all registered callbacks will be triggered
 * AND triggered after registration.
 */
EventManager.prototype.enhanceTrace = function () {
    logger.info('Start to enhance trace ...');
    debug = false;
    ...
    this.events.forEach(function (event) {
        ...
        //TODO: FSREQWRAP?
        /**
         * fix bug: do not create the virtual event for last FSREQWRAP event
         */
        if (event.resourceType != Event.GLOBAL_RESOURCETYPE && !event.hasOwnProperty('triggerIO')) {
            var targetEvent = event.id,
                priorEvent = event.prior,
                virtualEvent = new VirtualEvent('T' + targetEvent, targetEvent, priorEvent);
            that.virtualEvents.set(virtualEvent.id, virtualEvent);
            //console.log('create virtual event ' + virtualEvent.id);
        }
    })
};
 ...
 * @param {String} prior 
 */
function VirtualEvent (id, targetId, prior) {
    this.id = id;
    this.targetEvent = targetId;
    this.prior = prior;
};

VirtualEvent.prototype.addEdge = function (edgeType, nextEventId) {
    if (!this.hasOwnProperty('edges')) {
        this.edges = {};
    }
    if (!this.edges.hasOwnProperty(edgeType)) {
 ...
 * @param {Number} id: lineno
 * @param {String} prior
 * @param {String} follower
 */
function EventOperation (type, id, prior, follower) {
    this.type = type;
    this.id = id;
    this.prior = prior;
