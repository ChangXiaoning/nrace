...
EventManager.prototype.enhanceTrace = function () {
    ...
    this.events.forEach(function (event) {
        ...
        //TODO: FSREQWRAP?
        if (event.resourceType != Event.GLOBAL_RESOURCETYPE || event.hasOwnProperty('prior')) {
            var targetEvent = event.id,
                priorEvent = event.prior,
                virtualEvent = new VirtualEvent(targetEvent, priorEvent);
            that.virtualEvents.set(event.id, virtualEvent);
        }
    })
};
 ...
 * @param {String} prior 
 */
function VirtualEvent (targetEvent, prior) {
    this.id = targetEvent;
