    ...
    function processLine (line, last) {
        ...
        if (line) {
            ...
            if (entryType == LogEntryType['SOURCE_MAPPING']){
                ...
                record = new ObjectRecord(lineno, metadata[4], objAccessType, metadata[1], that.locationManager.query(metadata[1]));
                ...
                //log format: entryType, iid, scope, name, objId
                var objAccessType = AccessType.entryType2accessType(entryTypeName);
                record = new ObjectRecord(lineno, metadata[3], objAccessType, metadata[0], that.eventManager.top(), that.locationManager.query(metadata[2]));
 ...
 * accessType can be:
 * DECLARE_OBJ, ASSIGN_OBJ, USE_OBJ, WRITE_PROP, READ_PROP
 * CREATE_OBJ, USE_OBJ, PUTFIELD, GETFIELD
