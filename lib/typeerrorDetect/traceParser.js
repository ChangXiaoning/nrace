...
TraceParser.prototype.parse = function (traceFile, cb){
    ...
    function computePrior (metadata) {
        console.log('computePrior for %s:', metadata[1]);
        //console.log('objects: %s', builder.toString());
        var candidates = builder.getAll().filter(e => {return e.id == metadata[3];});
        if (candidates.length === 0){
            console.log('res: %s', metadata[3]);
            return metadata[3];
        }
        let current = builder.getCurrentEvent();
        console.log('current: %s', current);
        console.log('res: %s', metadata[3] === current? metadata[3] : current);
        return metadata[3] === current? metadata[3] : current;
    }
};
