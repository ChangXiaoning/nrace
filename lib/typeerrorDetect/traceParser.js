...
EventManager.prototype.enter = function (event) {
    this.stack.push(event.id);
};

EventManager.prototype.exit = function (event) {
    if (this.stack.top != event.id) {
        logger.error('Something wrong event exiting does not equal to the top of stack.');
    } else {
        this.stack.pop();
    }
};
