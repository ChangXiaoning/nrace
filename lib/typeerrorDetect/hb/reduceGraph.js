...
function reduceGraph (asyncObjects, relations, actions) {
    ...
    for (let i = 0; i < newObjs.length - 1; i++) {
        for (let j = i + 1; j < newObjs.length; j++) {
            ...
            if (!nr.happensBefore(ei.id, ej.id) && 
                !nr.happensBefore(ej.id, ei.id)) {
                if (relations.happensBefore(ei.id, ej.id)) {
                    nr.add(ei.id, ej.id, '');
                } else if (relations.happensBefore(ej.id, ei.id)) {
                    nr.add(ej.id, ei.id, '');
                }
            }
        }
