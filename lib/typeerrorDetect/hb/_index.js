...
function buildHBGraph (args) {
    ...
    if (isbuildGraph) {
        applyRules._applyRules(asyncObjects, relations, actions);

        //reduced graph only with callback nodes
        let rg = reduceGraph(asyncObjects, relations, actions);
        ...
        if (args.image) {
            ...
            //graphviz.drawGraph(figName, {nodes: relations.nodes, edges: relations.hb});
            
            figName = args.file.replace('.log', '.hb-graph-only-cbs.png');
            console.log(figName);
            graphviz.drawGraph(figName, { nodes: rg.relations.nodes, edges: rg.relations.hb });
        }
    }
