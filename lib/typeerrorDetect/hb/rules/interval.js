...
function apply(intervalHB, e){
    ...
    if (e.type === 'Timeout'){
        for (let key in intervalHB) {
            ...
            if (idx> -1) result.push(intervalHB[idx - 1]);
        }
    }
    return result;
}
    ...
    for (let i = 0; i < intervals.length - 1; i++) {
        ...
        for (let j = i + 1; j < intervals.length; j++) {
            ...
            if (basei == basej && offseti < offsetj) {
                intervalHB[intervals[i].id] = intervalHB[intervals[i].id] ? intervalHB[intervals[i].id] : [];
                intervalHB[intervals[i].id].push(intervals[j].id);
            }
        }
    }
    return intervalHB;
