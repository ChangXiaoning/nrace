...
function _applyRules (asyncObjs, relations, actions) {
    ...
    new_asyncObjs.getAll().forEach(t => {
        ...
        let priors = collectPrior(asyncObjs, t, intervalHB, relations.promiseAllSet, relations.promiseRaceSet);
...
function collectPrior (asyncObjs, e, intervalHB, promiseAllSet, promiseRaceSet) {
    //if (e.id == '*A*150')
    //console.log(e.id);
    var priors = [];
    var candidate = null;
