...
class Relations {
    ...
    happensBefore(aoi, aoj) {
        ...
        if (icid == jcid) {
            return ichain.indexOf(aoi) < jchain.indexOf(aoj);
        }
        let directSync = this.getLastSync(icid, jcid);
        if (directSync) {
            return ichain.indexOf(aoi) <= ichain.indexOf(directSync.fore) && jchain.indexOf(directSync.later) <= jchain.indexOf(aoj);
        }
        ...
        if (syncs)
            for (let sync of syncs) {
                if (ichain.indexOf(aoi) <= ichain.indexOf(sync.fore) && jchain.indexOf(sync.later) <= jchain.indexOf(aoj))
                    return true;
            }

        let jidx = this.getIdxInExecution(aoj);
        let visited = {};
        let rels = this.hb.filter(r => r.fore === aoi && this.isInSubGraph(jidx, r.fore));
        ...
        while (rels.length > 0) {
            ...
            if (!visited[relation.later]) {
                ...
                if (relation.later === aoj)
                    ...
                    let ind_rels = this.hb.filter(r => r.fore === relation.later && this.isInSubGraph(jidx, r.fore));
