...
class Relations {
    constructor (asyncObjs, promiseAllSet, promiseRaceSet, hb, chains, sync, actions) {
        ...
        if (arguments.length == 3) {
            ...
            this.initNodes();
            this.matrix = new Matrix(this.events_num);
            this.hb_check = 0;
            this.hb_call = 0;
            this.iteration = 0
        }
    ...
    happensBefore(aoi, aoj) {
        ...
        if (icid == jcid) {
            ...
            if (result) this.matrix.update(aoi, aoj, 1);
            return result;
        }
        ...
        let visited = {};
        let rels = this.hb.filter(r => r.fore === aoi && this.isInSubGraph(jidx, r.later));
        ...
        while (rels.length > 0) {
            ...
            if (!visited[relation.later]) {
                ...
                if (relation.later === aoj) {
                    ...
                    let ind_rels = this.hb.filter(r => r.fore === relation.later && this.isInSubGraph(jidx, r.later));
