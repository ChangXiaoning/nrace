...
HappensBeforeGraph.prototype.addRegister2Trigger = function () {
    ...
    this.eventNodes.forEach(function (event) {
        ...
        if (!event.hasOwnProperty('triggerIO')) {
            ...
            if (prior != undefined) {
                var triggerId = that.virtualEvents.get('T' + event.id).id;
...
HappensBeforeGraph.prototype.addTrigger2Cb = function () {
    ...
    this.eventNodes.forEach(function (event) {
        ...
        if (event.resourceType != Event.GLOBAL_RESOURCETYPE && !event.hasOwnProperty('triggerIO')) {
            var trigger = that.virtualEvents.get('T' + event.id);
...
HappensBeforeGraph.prototype.addFIFO = function () {
    for (var i = 0; i < this.eventNodes.length - 1; i++) {
        ...
        if (!eventI || eventI.resourceType == Event.GLOBAL_RESOURCETYPE) continue;
        var triggerI = this.virtualEvents.get('T' + eventI.id),
            priorityI = eventI.priority;
        for (var j = i + 1; j < this.eventNodes.length; j++) {
            var eventJ = this.eventNodes[j];
            if (!eventJ || eventJ.resourceType == Event.GLOBAL_RESOURCETYPE || eventJ.priority != priorityI) continue;
            console.log('i: ' + i + ', j: ' + j);
            var triggerJ = this.virtualEvents.get('T' + eventJ.id);
            /** TODO: not add edge between two events on the same callback chain */
            if (this.isHappensBefore(triggerI.id, triggerJ.id)) {
 ...
 * @param {String} eventJ
 * @returns {Boolean}
 * Bug: how to model FIFO??gi?
 */
HappensBeforeGraph.prototype.isHappensBefore = function (eventI, eventJ) {
    /** Check if eventI exists */
    if (!this.eventNodes[eventI] && !this.virtualEvents.has(eventI)) {
        logger.error('There not exist event #' + eventI + ' (I)');
        return;
    }
    /*
    console.log('ishb');
    for (var [id, event] of this.virtualEvents.entries()) {
        debugHelper('==========' + id);
        //writeObj(event);
    }
    console.log('*********');
    debugHelper(this.virtualEvents);
    console.log(eventJ);
    console.log(typeof(eventJ));*/
    /** Check if eventJ exists */
    //console.log('!this.virtualEvents.has[eventJ]', !this.virtualEvents.has[eventJ]);
    if (!this.virtualEvents.has(eventJ)) {
        logger.error('There not exist event #' + eventJ + ' (J)');
        return;
    }
    var event;
    if (eventI.indexOf('T') == -1) {
        event = this.eventNodes[eventI];
    } else {
        event = this.virtualEvents.get(eventI);
    }
