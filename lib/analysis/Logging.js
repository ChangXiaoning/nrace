...
var AbstractFluentTracer = (function (_super) {
    ...
    AbstractFluentTracer.prototype.logPutfield = function (iid, baseObjId, propName, valObjId, isOpAssign, valIsObject) {
        if (!this.beforeLog(iid))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["PUTFIELD"], iid).writeInt(baseObjId).writeString(propName).writeInt(valObjId).writeByte(isOpAssign).writeByte(valIsObject).writeRt();
    };
    ...
    AbstractFluentTracer.prototype.logGetfield = function (iid, baseObjId, propName, valObjId, valIsObject) {
        if (!this.beforeLog(iid))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["GETFIELD"], iid).writeInt(baseObjId).writeString(propName).writeInt(valObjId).writeByte(valIsObject).writeRt();
    }
