...
var AbstractFluentTracer = (function (_super) {
    ...
    AbstractFluentTracer.prototype.logPutfield = function (iid, baseObjId, propName, valObjId, basename) {
        if (!this.beforeLog(iid))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["PUTFIELD"], iid).writeInt(baseObjId).writeString(propName).writeInt(valObjId).writeString(basename).writeRt();
    };
