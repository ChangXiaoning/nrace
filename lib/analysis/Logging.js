...
var AbstractFluentTracer = (function (_super) {
    ...
    AbstractFluentTracer.prototype.logPutfield = function (iid, baseObjId, propName, valObjId, isOpAssign, accessPath, rightval) {
        if (!this.beforeLog(iid))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["PUTFIELD"], iid).writeInt(baseObjId).writeString(propName).writeInt(valObjId).writeByte(isOpAssign).writeString(accessPath).writeInt(rightval).writeRt();
    };
    ...
    AbstractFluentTracer.prototype.logGetfield = function (iid, baseObjId, propName, valObjId, accessPath) {
        if (!this.beforeLog(iid))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["GETFIELD"], iid).writeInt(baseObjId).writeString(propName).writeInt(valObjId).writeString(accessPath).writeRt();
    }
    AbstractFluentTracer.prototype.logWrite = function (iid, refId, name, objId, accessPath, rightval) {
        if (!this.beforeLog(iid))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["WRITE"], iid).writeInt(refId).writeString(name).writeInt(objId).writeString(accessPath).writeInt(rightval).writeRt().logCallStack(['W', iid, refId, name]);
    };
