...
var AbstractFluentTracer = (function (_super) {
    ...
    AbstractFluentTracer.prototype.logBinary = function (iid, op, left, right, result, isOpAssign) {
        if (!this.beforeLog())
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["BINARY"], iid).writeString(op).writeInt(left).writeInt(right).writeInt(result).writeString(isOpAssign).writeRt();
    }
    ...
    AbstractFluentTracer.prototype.logInvokeFun = function (iid, fName, argsFun) {
        ...
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["INVOKE_FUN"], iid).writeString(fName).writeInt(argsFun.length);
        for (let i = 0; i < argsFun.length; i++) {
            this.writeInt(argsFun[i]);
        }
        this.writeRt();
        /*
    ...
    AbstractFluentTracer.prototype.logConditional = function (iid, result) {
        if (!this.beforeLog())
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["CONDITIONAL"], iid).writeByte(result).writeRt();
    }
    ...
    AbstractFluentTracer.prototype.logFunctionEnter = function (iid, fName, current, funId) {
        if (!this.beforeLog() || !LogEntryType.hasOwnProperty('FUNCTION_ENTER'))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["FUNCTION_ENTER"], iid).writeString(fName).writeInt(current).writeInt(funId).writeRt();
    };
    AbstractFluentTracer.prototype.logFunctionArg = function (iid, funId) {
        if (!this.beforeLog() || !LogEntryType.hasOwnProperty("FUNCTION_ARG"))
            return;
        this.flushIfNeeded().writeTypeAndIID(LogEntryType["FUNCTION_ARG"], iid).writeInt(funId).writeRt();
    };
    ...
    AbstractFluentTracer.prototype.logFreeVars = function (iid, names) {
        ...
        if (typeof names === 'string') {
            ...
            this.flushIfNeeded().writeTypeAndIID(LogEntryType["FREE_VARS"], iid).writeInt(names.length);
            for (var i = 0; i < names.length; i++) {
