...
function NativeModels(idManager, tracer) {
     ...
     */
    this.invokeFunPreNatives = allocMap();
    /**
     * native functions that we model after they have been called, i.e., in invokeFun()
     */
    this.invokeFunPostNatives = allocMap();
    this.callbackIdToGlobal = allocMap();
    ...
    this.nativeFunctionModels = {
        ...
        'Array.prototype.concat': function (iid, f, base, args, val, isConstructor, name) {
            ...
            if (val) {
                ...
                var valId = _this.idManager.findOrCreateUniqueId(val, iid, true);
                var len = val.length;
        ...
        'setInterval': function (iid, f, base, args, val, isConstructor, name) {
            ...
            var timeoutId = val;
            _this.callbackIdToGlobal.set(timeoutId, globalName);
            _this.tracer.logWrite(iid, globalName, _this.idManager.findOrCreateUniqueId(intervalFun, iid, true));
        },
        'Object.defineProperty': function (iid, f, base, args, val, isConstructor, name) {
            ...
            if (targetObj && util.isObject(targetObj) && property && descriptor) {
                var targetId = _this.idManager.findOrCreateUniqueId(targetObj, iid, true);
                if (util.HOP(descriptor, 'value')) {
                ...
                if (util.HOP(descriptor, 'get')) {
                    var getter = descriptor.get;
                    var getterId = _this.idManager.findOrCreateUniqueId(getter, iid, true);
                    _this.tracer.logPutfield(iid, targetId, "~get~" + property, getterId);
                }
                if (util.HOP(descriptor, 'set')) {
                    var setter = descriptor.set;
                    var setterId = _this.idManager.findOrCreateUniqueId(setter, iid, true);
        ...
        'EventEmitter.prototype.on': function(iid, f, base, args, val, isConstructor, name){
            var baseId = _this.idManager.findObjId(base);
            //xiaoning: in order to log createObj for listener function
            var listenerFunId = _this.idManager.findObjId(args[1]);
            _this.tracer.logPutfield(iid, baseId, args?args[0]:undefined);
        },
    ...
    if (this.invokeFunPreNatives.has(f)) {
        if (f === Array.prototype.splice) {
            ...
            if (typeof fun === 'function') {
                var funIID = util.getFunEnterIID(fun);
                ...
                this.tracer.logWrite(iid, freshGlobal, this.idManager.findOrCreateUniqueId(fun, iid, true));
                args[0] = wrapper;
            }
        }
        return true;
