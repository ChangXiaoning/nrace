...
(function (___TraceCollector___) {
    ...
    var AbstractFluentTracer = (function (_super) {
        ...
        AbstractFluentTracer.prototype.logFs = function(ref, funName, args, location, fd, file, stat){
            ...
            function _logFs (entryType, resource, ref, funName, location){
                if(!that.beforeLog(location))
                    return;
                if(typeof resource == 'string')
                    resource = path.resolve(process.cwd(), resource);
                that.flushIfNeeded().writeType(LogEntryType[entryType]).writeString(resource).writeString(ref).writeString(funName).writeInt(lastAsyncId).writeString(location).writeRt();
                lastAsyncId = -1;
            }

            function _doLog (accessType, resource){
                if(accessType =='OPEN'){
                    if(stat && stat.exists == false){
                        _logFs('FS_CREATE', args[0], ref, funName, location);
                    }
                    ...
                    _logFs('FS_OPEN',args[0], ref, funName, location);
                    //_logFs('FS_'+accessType,args[0], ref, funName, location);
                }else{
                    //just record current record 
                    _logFs('FS_'+accessType, resource, ref, funName, location);
                }
            }
            ...
            if(ref == 'fs'){
                ...
                if(funName == 'emit'){
                    if(args[0] =='data'){
                        _logFs('FS_READ', file, ref, funName+'_'+args[0], location);
                    }else if(args[0] == 'close'){
                        _logFs('FS_STAT_CLOSED', file, ref, funName+'_'+args[0], location);
                    }
                }else if(funName == 'pipe'){
                    _logFs('FS_READ', file, ref, funName, location);
                    _logFs('FS_WRITE', args[0]._file, ref, funName, location);
                ...
                if(funName == 'emit'){
                    ...
                    _logFs('FS_WRITE', file, ref, funName, location);
                } 
            }
        }
        AbstractFluentTracer.prototype.logFunctionEnter = function (iid, funObjId) {
            if (!this.beforeLog() || !LogEntryType.hasOwnProperty('FUNCTION_ENTER'))
                return;
            this.flushIfNeeded().writeTypeAndIID(LogEntryType["FUNCTION_ENTER"], iid).writeRt();
        };
...
(function (___TraceCollector___) {
    ...
    var TraceCollector = (function () {
        ...
        TraceCollector.prototype.invokeFun = function (iid, f, base, args, val, isConstructor, isMethod) {
            ...
            //xiaoning
            //console.debug('invokeFun %s(%s)', f, args);
            var fString=f.toString();
            var fName=fString.slice(fString.indexOf('function')+9, fString.indexOf('function')+19);
            console.debug('*********invokeFun %s', f);
            //console.debug(f instanceof Function);
            console.debug('f.name: %s', fName);
            //console.debug('@@@@@@@@@@ %s', typeof(JSON.stringify(f)));
            //console.debug('@@@@@@@@@@ %s', JSON.stringify(f));
            var description='';
            for (var i in args){
                description+=i+'='+args[i]+'\n';
                //console.debug(args[i]);
            }
            console.debug('arguments:\n', description);
            if (fName=='setTimeout'){
                console.debug('~~~~~~~~~~invoke setTimeout');
            }
       };
        ...
        TraceCollector.prototype.read = function (iid, name, val, isGlobal, isScriptLocal, refId){
            ...
            //xiaoning
            console.debug('**********read %s: %s', name, val);
            //fs.writeSync(1, '**********read %s: %s', name, val);
        };
