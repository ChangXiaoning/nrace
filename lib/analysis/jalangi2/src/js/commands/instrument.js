...
(function (sandbox) {
    ...
    function instrument(options, cb) {
        ...
        function rewriteInlineScript(src, metadata) {
            ...
            if(instResult.skip!=true){
                instrumentedCode = instUtil.applyASTHandler(instResult, astHandler, sandbox);
            ...
            if (instResult) {
                ...
                if(instResult.skip != true){
                    ...
                    if (instrumentCodeAfter) {
                        var aret = instrumentCodeAfter(undefined, instResult.code);
                        if (aret) {
                            instrumentedCode = instResult.code = aret.result;
                        }
                    }
        ...
        function transform(readStream, writeStream, file) {
            ...
            if(sandbox.configUtil && typeof sandbox.configUtil.skipInstrument == 'function'){
                if(sandbox.configUtil.skipInstrument(file.name) == true){
            ...
            if (extension === '.html') {
                ...
                var instrumentJSFile = file.name.indexOf(EXTRA_SCRIPTS_DIR) !== -1 ||
                    ...
                    (!onlyIncludeList && (!excludePattern || file.name.match(excludePattern) == null));
                if (instrumentJSFile) {
...
//                console.log("excluding " + file.name);
        ...
        var copyJalangiRuntime = function () {
            ...
            var copyFile = function (srcFile) {
                if (jalangiRoot) {
                    srcFile = path.resolve(jalangiRoot, srcFile);
                }
