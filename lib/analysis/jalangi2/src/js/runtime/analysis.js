...
(function (sandbox) {
    ...
    function N(iid, name, val, flags, refId) {
        ...
        if (!bFlags[1] && sandbox.analysis && sandbox.analysis.declare) {
            if (bFlags[0] && argIndex > 1) {
                sandbox.analysis.declare(iid, name, val, bFlags[0], argIndex - 2, bFlags[2], refId);
            } else {
                sandbox.analysis.declare(iid, name, val, bFlags[0], -1, bFlags[2], refId);
            }
            if (aret) {
                val = aret.result;
            }
    ...
    //xiaoning: add basename
    function G(iid, base, offset, flags, basename) {
        ...
        if (sandbox.analysis && sandbox.analysis.getField) {
            aret = sandbox.analysis.getField(iid, base, offset, val, bFlags[0], bFlags[1], bFlags[2], basename);
            if (aret) {
                val = aret.result;
            }
        }
    ...
    //xiaoning: add basename
    function P(iid, base, offset, val, flags, basename) {
        ...
        if (sandbox.analysis && sandbox.analysis.putField) {
            aret = sandbox.analysis.putField(iid, base, offset, val, bFlags[0], !!bFlags[1], basename);
            if (aret) {
                val = aret.result;
            }
        }
