...
//        StatCollector.suspendTimer("replace");
    ...
    function wrapPutField(node, base, offset, rvalue, isComputed) {
        if (!Config.INSTR_PUTFIELD || Config.INSTR_PUTFIELD(isComputed ? null : offset.value, node)) {
            printIidToLoc(node);
            //xiaoning: to get the identifier (name) of base
            var baseNode = base.arguments.find(node => node.type == 'Identifier');
            var baseName = null;
            if (baseNode) {
                baseName = baseNode.name;
            }
            //xiaoning: create a literal node
            var newBaseNameNode = {type: 'Literal', value: baseName.toString()};
            
            var ret = replaceInExpr(
                logPutFieldFunName +
                "(" + RP + "1, " + RP + "2, " + RP + "3, " + RP + "4," + (createBitPattern(isComputed, false))  + ", " + RP + "5)",
                getIid(),
                base,
                offset,
                rvalue,
    ...
    function wrapGetField(node, base, offset, isComputed) {
        if (!Config.INSTR_GETFIELD || Config.INSTR_GETFIELD(node.computed ? null : offset.value, node)) {
            ...
            var ret = replaceInExpr(
                logGetFieldFunName + "(" + RP + "1, " + RP + "2, " + RP + "3," + (createBitPattern(isComputed,false, false)) + ", " + RP + "4)",
                getIid(),
                base,
                offset,
                newBaseNameNode
                //replaceInExpr(baseName)
            );
            transferLoc(ret, node);
            return ret;
        } else {
    ...
    function wrapRead(node, name, val, isReUseIid, isGlobal, isScriptLocal) {
        if (!Config.INSTR_READ || Config.INSTR_READ(name, node)) {
            printIidToLoc(node);
            scope.iid = isReUseIid ? getPrevIidNoInc() : getIid();
            scopeId2Iid[scope.id] = [scope.iid.value, node.type];
            var ret = replaceInExpr(
                logReadFunName + "(" + RP + "1, " + RP + "2, " + RP + "3," + (createBitPattern(isGlobal,isScriptLocal)) + ", "+ RP +"4)",
                scope.iid, 
                name,
                val,
                replaceInExpr(scope.getOwnerScopeInfo(name.value))
            );
            transferLoc(ret, node);
            return ret;
        } else {
...
//        }
    ...
    function wrapWrite(node, name, val, lhs, isGlobal, isScriptLocal, isDeclaration, scopeInfo) {
    ...
    function wrapWriteWithUndefinedCheck(node, name, val, lhs, scopeInfo) {
        if (!Config.INSTR_WRITE || Config.INSTR_WRITE(name, node)) {
            printIidToLoc(node);
...
//        );
            var ret = replaceInExpr(
                logWriteFunName + "(" + RP + "1, " + RP + "2, " + RP + "3, " + logIFunName + "(typeof(" + lhs.name + ")==='undefined'?undefined:" + lhs.name + ")," + createBitPattern(true, false, false) +", "+RP+"4)",
                getIid(),
                name,
                val,
                replaceInExpr(scopeInfo)
        ...
        if (node.left.type === 'Identifier') {
            if (scope.hasVar(node.left.name)) {
                ret = wrapWrite(node.right, createLiteralAst(node.left.name), node.right, node.left, false, scope.isGlobal(node.left.name), isDeclaration,
                    scope.getOwnerScopeInfo(node.left.name));
            } else {
                ret = wrapWriteWithUndefinedCheck(node.right, createLiteralAst(node.left.name), node.right, node.left, scope.getOwnerScopeInfo(node.left.name));
    ...
    function instrumentLoadModStore(node, isNumber) {
        if (node.left.type === 'Identifier') {
            ...
            if (scope.hasVar(node.left.name)) {
                ...
                tmp2 = wrapWriteWithUndefinedCheck(node.right, createLiteralAst(node.left.name), tmp1, node.left, scope.getOwnerScopeInfo(node.left.name));
            ...
            var declarations = MAP(node.declarations, function (def) {
                if (def.init !== null) {
                    var init = wrapWrite(def.init, createLiteralAst(def.id.name), def.init, def.id, false, scope.isGlobal(def.id.name), true, 
                        scope.getOwnerScopeInfo(def.id.name));
...
//                var ret = prependScriptBody(node, body);
    ...
    function addScopes(ast) {
        ...
        Scope.prototype.ownerScope = function (name){
            var s = this;
            while (s !== null) {
                if (HOP(s.vars, name)){
                    return s;
        ...
        Scope.prototype.getOwnerScopeInfo = function (name){
            if(typeof name == 'object')
                name = name.value;
            var owner = this.ownerScope(name);
            if(owner){
                if(!owner.iid){
                    return '-1';
                }
                return ''+owner.id+'';
                //return 'J$.R('+owner.id+',"'+name+'")';
                //return JSON.stringify([owner.id, owner.vars[name]])
            }else{
                return 0+'';
                //return 'J$.R(0, "'+name+'")';
...
//        console.log(JSON.stringify(newAst,null,"  "));
    ...
    function initializeIIDCounters(forEval) {
        J$.initialIid = J$.initialIid || 0;
        var adj = J$.initialIid;
    ...
    function instrumentCode(options) {
        ...
        if(!options.rootDir){
            options.rootDir = path.dirname(options.instCodeFileName);
        }

        instCodeFileName = options.instCodeFileName ? path.relative(options.rootDir, options.instCodeFileName) : (options.isDirect?"eval":"evalIndirect");
        origCodeFileName = options.origCodeFileName ? path.relative(options.rootDir, options.origCodeFileName) : (options.isDirect?"eval":"evalIndirect");
        var instrumentCodePre = (sandbox.configUtil&&sandbox.configUtil.instrumentCodePre)?sandbox.configUtil.instrumentCodePre:(sandbox.analysis && sandbox.analysis.instrumentCodePre)?sandbox.analysis.instrumentCodePre:undefined;
        if (instrumentCodePre) {
            aret = instrumentCodePre(thisIid, code, options.isDirect, instCodeFileName, origCodeFileName);
            if (aret) {
                code = aret.code;
                skip = aret.skip;
            }
        ...
        if (typeof code === 'string' && code.indexOf(noInstr) < 0) {
            try {
                ...
                console.log("\n\nFailed to instrument "+origCodeFileName+": "+ex.message+"\n");
        ...
        instCode = JALANGI_VAR +'.scopes = '+ JSON.stringify(scopeId2Iid) +';\n' +code;
        ...
        var instrumentCodeAfter = (sandbox.configUtil&&sandbox.configUtil.instrumentCode)?sandbox.configUtil.instrumentCode:(sandbox.analysis && sandbox.analysis.instrumentCode)?sandbox.analysis.instrumentCode:undefined;
        if (instrumentCodeAfter) {
            aret = instrumentCodeAfter(thisIid, instCode, newAst, options.isDirect);
            if (aret) {
