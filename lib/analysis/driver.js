...
cb=cb||function () {/*do nothing*/};
    ...
    var traceFile=parserd.filepath;
    if (!traceFile.endsWith('.log') && traceFile.length<128 && fs.statSync(traceFile).isDirectory()) {
        traceFile=path.resolve(traceFile,'./ascii-trace.log');
    }

    pythonScript=__dirname+'/TraceParser.py';
    cliArgs=[pythonScript, traceFile];
    runPythonProg('python',cliArgs, 'parse into JSON file');
};
    ...
    switch (args[0]) {
        ...
        default:
            var msg = 'Surppoted commands:\n' +
                ...
                '   parse path/to/traceFile    --parse the trace file into python objects and store into JSON file\n';
